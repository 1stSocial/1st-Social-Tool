    ...
    <tbody>
        <? if (!empty($theme_data)): foreach ($theme_data as $Theme): ?>
                ...
                <tr>
                    ...
                    <td><div class="btn-group"> <a href="<?php echo site_url('/admin/setting/edit_theme/' . $Theme->id) ?>" class="btn btn-primary"><i class="icon-edit icon-white"></i> Edit</a><a onclick="return confirm('Are you sure?')" href="<?php echo site_url('/admin/setting/delete_theme/' . $Theme->id) ?>" class="btn btn-danger"><i class="icon-trash icon-white"></i> Delete</a> </div></td>
                </tr>
            <? endforeach;
        endif; ?>
    </tbody>
